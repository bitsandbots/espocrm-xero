...
export default class XeroIntegrationView extends IntegrationsEditView {
    ...
    actionPingXero() {
        ...
        Espo.Ajax.getRequest('XeroIntegration/ping')
            .then(data => {
                if (data && data.ok) {
            ...
            .catch(() => Espo.Ui.error('Ping request failed. Check server logs.'))
            .finally(() => $btn.prop('disabled', false).text('Check Connection'));
    }

    actionDisconnectXero() {
        Espo.Ui.confirm(
            'Disconnect from Xero? Tokens will be cleared. Sync will stop until you reconnect.',
            {confirmText: 'Disconnect', cancelText: 'Cancel'},
            () => {
                Espo.Ajax.request('XeroIntegration/connection', 'DELETE')
                    .then(() => {
